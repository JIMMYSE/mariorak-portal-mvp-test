...
const { data: postDetailData } = useNewsDetail(newsId);
const { options } = useCommonCode('POST_TY');
const postType = computed(
  () => options.value.find((item: Option) => item.value === postDetailData.value?.clum_cate_cd)?.label
);
...
const { data: commentList, refetch } = useNewsCommentList({
  postId: newsId,
...
const onRegisterCommnet = () => {
  useCreateNewsComment(newsId.value, newCommnet.value);
  refetch();
  newCommnet.value = '';
};
...
const onRegisterReply = (clum_cmmt_id: number) => {
  useCreateNewsReply(newsId.value, replyCommnet.value, clum_cmmt_id);
  refetch();
  replyCommnet.value = '';
};
</script>
  ...
  <section class="px-6">
    <div class="py-[16px] border-b border-[#f0f0f0]">
      <div class="flex justify-between items-end">
        <!-- <div>
          <span class="badge font-medium" v-for="badge in postDetailData?.tag_list" :key="badge">{{ badge }}</span>
        </div> -->
        <div>
          ...
          <p class="text-[#767676] text-sm font-normal leading-tight">
            {{ formatDate(postDetailData?.created_at) }} {{ postDetailData?.admn_name }}
          </p>
        </div>
        <div>
  ...
  <section>
    <div class="flex justify-between">
      <c-btn
        ...
        flat
        :color="postDetailData?.prev_id ? 'primary' : 'grey-3'"
        :disabled="!postDetailData?.prev_id"
        @click="onClickPost(postDetailData?.prev_id)"
        ><c-icon
          name="icon_preview_arrow"
          size="16px"
          :color="postDetailData?.prev_id ? '#056bf1' : '#767676'"
          :fill="false"
        />이전글
      </c-btn>
      <c-btn
        class="enter_btn rounded-[30px] font-semibold text-base"
        flat
        :color="postDetailData?.next_id ? 'primary' : 'grey-3'"
        @click="onClickPost(postDetailData?.next_id)"
        >다음글<c-icon
          name="icon_enter_arrow"
          size="16px"
          :color="postDetailData?.next_id ? '#056bf1' : '#767676'"
          :fill="false"
        />
      </c-btn>
    </div>
  ...
  <section class="px-6 mt-[30px] pb-[50px]">
    ...
    >
      ...
      <c-btn
        ...
        color="grey-2"
        flat
        @click="onClickReplyInput(comment.clum_cmmt_id)"
        >{{ opendReplyInput == comment.clum_cmmt_id ? '답글접기' : '답글쓰기' }}
      </c-btn>
      <div class="bg-[#f7f7f7] rounded-[5px] pr-[10px] ml-5" v-if="opendReplyInput == comment.clum_cmmt_id">
        <div class="row no-wrap items-start">
          ...
          <c-btn class="p-0" flat @click="onRegisterReply(comment.clum_cmmt_id)"
