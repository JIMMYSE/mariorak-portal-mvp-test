...
const detail = ref({
  ...
  images: [
    { id: 1, imageUrl: '/images/dummy/popstore_dummy1.png', sortOrder: 1 },
    { id: 2, imageUrl: '/images/dummy/popstore_dummy2.png', sortOrder: 2 },
    { id: 3, imageUrl: '/images/dummy/popstore_dummy3.png', sortOrder: 3 },
    { id: 4, imageUrl: '/images/dummy/popstore_dummy4.png', sortOrder: 4 },
  ],
  goods: [
    { id: 1, title: '한정판 피규어', price: 10000, imageUrl: '/images/dummy/goods1.png' },
    { id: 2, title: '한정판 피규어', price: 10000, imageUrl: '/images/dummy/goods1.png' },
  ],
});
...
const sectionGoodsRef = ref<HTMLElement | null>(null);
...
const scrollToSection = (section: string) => {
  ...
  else if (section === 'goods') targetEl = sectionGoodsRef.value;

  if (targetEl) {
    const navHeight = 48;
    const headerHeight = 50;
    const gap = 20; // 버튼과 컨텐츠 사이 간격
    const offset = navHeight + headerHeight + gap;
    const top = targetEl.getBoundingClientRect().top + window.scrollY - offset;
    window.scrollTo({ top, behavior: 'smooth' });

    // 스크롤 애니메이션이 끝난 후 감지 재개
    setTimeout(() => {
...
const handleScroll = () => {
  ...
  // 버튼 클릭에 의한 스크롤 중이면 감지 건너뛰기
  if (isScrollingByClick.value) return;

  // 페이지 끝에 도달했는지 확인 (마지막 섹션 활성화)
  ...
  const offset = 150;
  const sections = [
    { name: 'goods', el: sectionGoodsRef.value },
...
<template>
  <q-page class="bg-white">
    <!-- ===== 상단 빨간 배경 영역 ===== -->
    <div class="bg-[#FF385C] pb-8">
      <!-- 페이지 타이틀 -->
      <div class="px-5 pt-4 pb-4">
        <h1 class="text-[32px] font-light text-white tracking-[-0.8px] leading-[20px]">POP-UP STORE</h1>
      </div>
        ...
        <div class="flex flex-col gap-[5px]">
          ...
          <div class="flex items-center mt-1">
            <div
              ...
              @click="$router.push({ name: 'popup-store-reserve', params: { id } })"
            >
      ...
      <!-- 스토어 안내 (빨간 배경 위 흰색 텍스트, 가운데 정렬) -->
      <div class="px-4 mt-6 text-white">
        <h3 class="text-[16px] font-semibold text-center tracking-[-0.4px] leading-[20px] mb-2">스토어 안내</h3>
        <p class="text-[14px] font-normal leading-[20px] tracking-[-0.35px]">{{ detail.description }}</p>
      </div>
    </div>

    <!-- ===== 섹션 네비게이션 (스크롤시 상단 고정) ===== -->
    <div ref="stickyNavRef" class="sticky top-[50px] z-10 bg-[#FF385C]">
      <div class="flex h-[48px]">
        <button
          v-for="section in ['info', 'image', 'goods']"
          :key="section"
    ...
    <div class="px-4 pb-[60vh]">
      ...
      <!-- goods 섹션 -->
      <div ref="sectionGoodsRef" class="pt-6">
        <h3 class="text-[24px] font-light text-[#4A5565] tracking-[-0.6px] leading-[20px] mb-5">goods</h3>
        <div class="grid grid-cols-2 gap-4">
          <div v-for="goods in detail.goods" :key="goods.id" class="flex flex-col gap-[5px]">
            <div
            ...
            >
              <q-img :src="goods.imageUrl" class="w-full h-full" fit="cover" />
            </div>
            <div class="leading-[20px] tracking-[-0.4px]">
              <p class="text-[16px] font-semibold text-[#4A5565]">{{ goods.title }}</p>
              <p class="text-[16px] font-light text-[#4A5565]">&#8361; {{ goods.price.toLocaleString() }}</p>
            </div>
          </div>
        </div>
      </div>
