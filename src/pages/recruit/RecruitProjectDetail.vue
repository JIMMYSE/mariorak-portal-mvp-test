...
const { data: projectDetail, refetch } = useProjectRecruitDetail(prjRcrtId);

const prjId = computed(() => toString(projectDetail.value?.prj_id));
...
const likeAction = createAction(() => {
  ...
  const onLikeProject = async () => {
    ...
    like.value ? await useLike('projectOrgame', prjId, 'project-detail') : await onUnlike(prjId.value);
    await refetch();
  };
...
// const { data: similarProjectData } = useSimilarProjectList(prjRcrtId);

const { enterRoom } = useBridge();
</script>
<template>
  <q-page v-if="projectDetail">
    <section>
      <div class="h-[210px] w-full">
        ...
        <div @click="likeAction.onLikeProject" class="absolute z-10 mr-[13px] right-0 top-2">
          <c-icon name="icon_heart_btn" size="30px" :active="likeAction.like.value" active-color="#EA2E2E" />
        </div>
      ...
      <div>
        ...
        <div class="mt-4">
          ...
          <c-btn
            ...
            @click="goTo(`/game-pack/project/${prjId}`)"
            >프로젝트 페이지로 이동하기
          </c-btn>
        </div>
      </div>
