...
const exhibitionName = ref('세가 레거시 전시전');
...
<template>
  <q-page class="bg-white px-4 pb-20">
    <h2 class="text-[18px] font-bold text-grey-5 mt-4 mb-3">{{ exhibitionName }}</h2>

    <div class="flex flex-col gap-1 mb-4">
      <div class="flex items-center gap-2">
        <span class="text-[14px] text-grey-3 w-[40px]">위치</span>
        <span class="text-[14px] text-grey-5">{{ exhibitionInfo.location }}</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[14px] text-grey-3 w-[40px]">기간</span>
        <span class="text-[14px] text-grey-5">{{ exhibitionInfo.period }}</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[14px] text-grey-3 w-[40px]">요금</span>
        <span class="text-[14px] text-grey-5">{{ exhibitionInfo.fee?.toLocaleString() }}원</span>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-[14px] text-grey-3 w-[40px]">링크</span>
        <span class="text-[14px] text-primary">{{ exhibitionInfo.linkUrl }}</span>
      </div>
    </div>
    ...
    <div class="border border-grey-1 rounded-lg p-4 mb-4">
      ...
      <div class="grid grid-cols-7 gap-1">
        <div
          ...
          :key="idx"
          class="text-center py-2 text-[14px] rounded-full cursor-pointer"
          :class="{
      ...
      <div class="border border-grey-1 rounded-lg p-4">
        <div class="flex justify-center gap-8">
          <div class="flex flex-col items-center gap-2">
            <span class="text-[14px] cursor-pointer" :class="selectedAmPm === '오전' ? 'text-grey-5 font-semibold' : 'text-grey-2'" @click="selectedAmPm = '오전'">오전</span>
            <span class="text-[14px] cursor-pointer" :class="selectedAmPm === '오후' ? 'text-grey-5 font-semibold' : 'text-grey-2'" @click="selectedAmPm = '오후'">오후</span>
          </div>
          <div class="flex flex-col items-center gap-2">
          ...
          <div class="flex flex-col items-center gap-2">
            ...
            <span class="text-[14px] text-grey-5 font-semibold bg-grey px-4 py-1 rounded">{{ String(selectedMinute).padStart(2, '0') }}</span>
    ...
    <div class="w-full border border-primary rounded-full py-3 text-center text-primary text-[16px] font-medium cursor-pointer mb-3" @click="onReserve">
      예약하기
    </div>
